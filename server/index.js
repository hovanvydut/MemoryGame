...
const GameModel = require('./models/database/games.model');
...
memoryGameNsp.on('connection', (socket) => {
  ...
  socket.on('create-room-memorygame', async ({ roomName, userInfo }, cb) => {
    const existRoom = await GameModel.findOne({ roomName });
    if (existRoom) {
  ...
  socket.on('join-room-memorygame', async ({ currentRoom, userInfo }, cb) => {
    const existRoom = await GameModel.findOne({ roomName: currentRoom });

    if (existRoom) {
      if (!existRoom.player1.id) {
        await GameModel.findByIdAndUpdate(existRoom._id, {
          player1: {
            id: userInfo.id,
            username: userInfo.username,
            elo: userInfo.elo,
        ...
        socket.emit('set-turn', 1);
      } else if (
        !existRoom.player2.id &&
        userInfo.id !== String(existRoom.player1.id)
      ) {
        await GameModel.findByIdAndUpdate(existRoom._id, {
          player2: {
            id: userInfo.id,
        ...
        userInfo.id !== String(existRoom.player1.id) &&
        userInfo.id !== String(existRoom.player2.id)
      ) {
        return cb({ message: 'room full' });
      }

      if (userInfo.id === String(existRoom.player1.id)) {
        socket.emit('set-turn', 1);
        socket.emit('get-info-other-player', { userInfo: existRoom.player2 });
      } else {
        socket.emit('set-turn', 2);
        socket.emit('get-info-other-player', { userInfo: existRoom.player1 });
      }

      socket.join(currentRoom);
      socket.emit('get-data-of-cards', { dataOfCard });
    } else {
      cb({ message: 'room is not exist' });
    }
  });
  ...
  socket.on('change-turn', (currentRoom) => {
    // await GameModel.findOneAndUpdate({roomName: currentRoom}, {turn: })
